...
import { pickNextThing } from './kodu.js';
...
test('pending order is an unpaid line', () => {
  const card = pickNextThing(vm({ order: { status: 'pending', subscriptionStatus: null } }));
  assert.equal(card.kind, 'unpaid');
...
test('the next visit is the card when nothing needs a decision', () => {
  const card = pickNextThing(vm({ upcomingBookings: [visit] }));
...
test('a pending new time opens the calendar', () => {
  const card = pickNextThing(vm({ upcomingBookings: [visit], pendingRescheduleIds: ['b1'] }));
  assert.equal(card.kind, 'visit');
...
test('away is the card only when nothing else is waiting', () => {
  const card = pickNextThing(vm({ extras: { requests: [], away: [{ from: '2026-04-01', to: '2026-04-12' }] } }));
